...
public:
    ...
    void Update(float DeltaTime, MapSearchNode GN){
        ...
        if(totalTime > moveTime && this->count < directions.size()){
            ...
            if(body.getPosition().x == (cellCenter+cellSize*GN.x) && body.getPosition().y == (cellCenter+cellSize*GN.y) && !NewPath){
                body.setTextureRect(sf::IntRect(textureSizeP.x * ic, textureSizeP.y * 0, textureSizeP.x, textureSizeP.y));
                ic++;

            }

            if(body.getPosition().x == (cellCenter+cellSize*directions[count]->x) && body.getPosition().y == (cellCenter+cellSize*directions[count]->y) && this->count < directions.size()-1){
                count++;
                c = 3;

            }

            if(body.getPosition().x != (cellCenter+ cellSize*directions[count]->x) && body.getPosition().y == (cellCenter+cellSize*directions[count]->y)) {

                if(body.getPosition().x > (cellCenter+ cellSize*directions[count]->x)) {
                    move(sf::Vector2f((cellCenter + cellSize * directions[count]->x) + 8 * c, (cellCenter + cellSize * directions[count]->y)));
            ...
            else if (body.getPosition().x == (cellCenter+ cellSize*directions[count]->x) && body.getPosition().y != (cellCenter+cellSize*directions[count]->y)){
                if(body.getPosition().y > (cellCenter+cellSize*directions[count]->y))
                    move(sf::Vector2f((cellCenter + cellSize * directions[count]->x), (cellCenter + cellSize * directions[count]->y) + 8 * c));
                else
                    move(sf::Vector2f((cellCenter + cellSize * directions[count]->x), (cellCenter + cellSize * directions[count]->y) - 8 * c));
